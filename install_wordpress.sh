...
apt-get update
apt-get install -y hhvm memcached libmemcached-dev
...
server {
    ...
    server_name localhost; # server_name domain.com
    include hhvm.conf;
    location / {
...
#https://www.rosehosting.com/blog/script-install-wordpress-on-a-debianubuntu-vps/
